class Solution:
    def combinationSum(self, nums: List[int], target: int) -> List[List[int]]:
        '''
            ...
            Key takeaways: 
                You you add an element to the list, you dont want to increment the index 
                    this is because in the problem you can have an unlimited number of the 
                    same value in a sublist, so we can keep adding untill the total is either
                    equal to the target or it its not then we can just break out of the loop 
        '''
        res = []
